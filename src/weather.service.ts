...
export class WeatherService {
  constructor(@InjectRepository(Weather) private weatherRepository: Repository<Weather>, private readonly configService:ConfigService) {}
  ...
  async getWeather(): Promise<GetWeatherDto[]> {
    ...
    const requestUrls = regionlds.map(
      (regionId) =>
        `https://apis.data.go.kr/1360000/VilageFcstMsgService/getLandFcst?serviceKey=${this.configService.get('WEATHER_API_KEY')}&pageNo=1&numOfRows=10&dataType=JSON&regId=${regionId}`,
    );
